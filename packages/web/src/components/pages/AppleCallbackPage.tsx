...
interface AppleCallbackPageProps {
  onGo?: (path: string) => void;
}

/**
 * Apple OAuth 콜백 페이지 (백엔드 주도 방식)
 *
 * 백엔드에서 OAuth 처리 후 리다이렉트되는 페이지입니다.
 * URL 쿼리 파라미터에서 stateId를 추출하여 인증 데이터를 조회합니다.
 */
export function AppleCallbackPage({ onGo }: AppleCallbackPageProps) {
  ...
  useEffect(() => {
    const processCallback = async () => {
      ...
      try {
        ...
        // URL 정리 (민감한 정보 제거)
        window.history.replaceState({}, document.title, '/auth/apple/callback');

        // 에러 처리
        if (error) {
          console.error('Apple OAuth 에러:', error, message);
          setErrorMessage(message || 'Apple 로그인에 실패했습니다.');
          setStatus('error');
          return;
        }
        ...
        // stateId 없으면 에러
        if (!stateId) {
          console.error('Apple OAuth: stateId가 없습니다');
        ...
        console.log('Apple OAuth: stateId로 인증 데이터 조회 중...');
        const response = await webApiService.auth.getAppleAuthData(stateId);
        ...
        if (response.token && response.user) {
          console.log('Apple 로그인 성공, 토큰 저장 중...');
        ...
        console.error('Apple 콜백 처리 중 오류:', error);
