      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-hidden">
        ...
        <div className="overflow-y-auto max-h-[calc(90vh-120px)] p-6">
          {loading ? (
                    ...
                    <div key={index} className="flex items-start gap-3 p-4 bg-blue-50 border border-blue-100 rounded-lg">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-4 mt-2 text-sm">
                          <span className="text-gray-600">수량: <span className="font-medium text-blue-600">{item.quantity || 0}개</span></span>
                ...
                <div>
                  ...
                  <div className="bg-gray-50 rounded-lg p-4 space-y-2">
                    ...
                    {(orderDetail.shippingAddress.street || orderDetail.shippingAddress.city) && (
                      <div className="flex items-start gap-3">
                        <span className="text-sm text-gray-500 min-w-[60px]">주소</span>
                  ...
                  <p className="text-xs text-gray-400 mt-2">* 개인정보 보호를 위해 일부 정보가 마스킹 처리되어 있습니다.</p>
                </div>
              )}
                  ...
                  <div className="relative">
                    ...
                    <div className="space-y-3">
                      {orderDetail.statusHistory.map((history, index) => {
                        const historyStatus = ORDER_STATUS_MAP[history.status] || { label: history.status, color: 'bg-gray-100 text-gray-800', icon: '•' };
                        return (
                          <div key={index} className="flex items-start gap-3 relative pl-6">
                            {/* 타임라인 점 */}
                            <div className={`absolute left-0 w-4 h-4 rounded-full border-2 border-white ${index === 0 ? 'bg-blue-500' : 'bg-gray-300'}`}></div>
            ...
            <div className="text-center py-12">
              <p className="text-gray-500">주문 정보가 없습니다.</p>
            </div>
          )}
        </div>
