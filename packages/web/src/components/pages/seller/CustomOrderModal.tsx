        ...
        <div className="overflow-y-auto max-h-[calc(90vh-120px)] p-6">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-muted">주문서를 불러오는 중...</p>
            </div>
          ) : error ? (
            ...
            <div className="space-y-6">
              ...
              <div className="pb-4 border-b border-gray-100">
                <p className="text-sm text-muted mb-1">주문서 제목</p>
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className="bg-surface-strong">
                          <th className="px-2 py-2 text-xs font-medium text-muted text-center w-12"></th>
                        ...
                        <tr className="border-b border-line">
                          ...
                          {Object.values((specs.sizes as any).left).map((size, idx) => (
                            <td key={idx} className="px-2 py-2.5 text-center">
                              <span className="inline-flex items-center justify-center w-8 h-8 bg-brand-100 text-brand-700 font-bold rounded-full text-sm">
                                {size as string}
                              </span>
                            </td>
                          ))}
                  ...
                  <div className="space-y-3">
                    {specs.desiredColor && (
                      <div className="flex items-start gap-3">
                        <span className="text-sm text-muted min-w-[80px]">원하는 색상</span>
                        <span className="text-sm font-medium text-gray-900">{specs.desiredColor}</span>
                      </div>
                    )}
                    {specs.desiredDate && (
                      <div className="flex items-start gap-3">
                        <span className="text-sm text-muted min-w-[80px]">수령 희망일</span>
                    ...
                    {specs.designNotes && (
                      <div>
                        <p className="text-sm text-muted mb-2">요청사항</p>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {specs.referenceImages.map((url, idx) => (
                      <a
                        ...
                        className="aspect-square rounded-lg overflow-hidden bg-surface hover:opacity-90 transition-opacity group"
                      >
                        <img
                          src={url}
                          alt={`참고 이미지 ${idx + 1}`}
            ...
            <div className="text-center py-12">
              <p className="text-muted">주문서 정보가 없습니다.</p>
            </div>
          )}
        </div>
