...
const AdminLayout: React.FC<AdminLayoutProps> = ({ children, currentUser, authLoading = false }) => {
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-surface">
        <div className="text-center">
          ...
          <div className="animate-pulse text-brand">로그인 페이지로 이동 중...</div>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-surface">
        <div className="text-center">
          ...
          <div className="animate-pulse text-brand">홈페이지로 이동 중...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-screen bg-surface">
      {/* 사이드바 */}
      <div className="w-64 bg-white shadow-sm border-r border-line">
        <div className="flex items-center justify-between h-16 px-4 bg-gradient-to-r from-brand to-brand-600 border-b">
        ...
        <nav className="mt-4 px-3">
          {navigation.map((item) => {
            ...
            return (
              ...
              >
                <IconComponent className={`w-5 h-5 mr-3 transition-colors ${isActive ? 'text-brand' : 'text-muted group-hover:text-gray-600'}`} />
                {item.name}
          ...
          >
            <FaHome className="w-5 h-5 mr-3 transition-colors text-muted group-hover:text-green-600" />
            쇼핑몰로 돌아가기
            <FaExternalLinkAlt className="w-4 h-4 ml-auto text-muted group-hover:text-green-600 transition-colors" />
          </button>
        </nav>

        <div className="absolute bottom-0 w-64 p-4 border-t border-gray-100 bg-surface/50">
          <div className="flex items-center space-x-3 bg-white rounded-xl p-3 shadow-sm border border-gray-100">
            <div className="flex-shrink-0">
              <div className="w-10 h-10 bg-gradient-to-r from-brand to-brand-600 rounded-full flex items-center justify-center shadow-sm">
